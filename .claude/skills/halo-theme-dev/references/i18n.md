...
Halo themes support i18n via `.properties` files under an `i18n/` directory.
...
> Halo uses `default.properties` as the fallback when no locale-specific file matches.
...
> Note: `[(#{key})]` is Thymeleaf's unescaped inlining syntax.
